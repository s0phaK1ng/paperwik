...
**v0.6.0 (April 2026).** Current release. First target user is a non-technical Windows user doing heavy Gemini Deep Research. Free and open source; no ads, no subscriptions beyond your own Claude Pro/Max plan. The v0.6.0 ship adds local zero-shot classification: a DeBERTa-v3-zeroshot-v2.0-c ONNX model (lazy-quantized to ~150 MB INT8 on first ingest) routes new sources by topical match against per-project descriptive labels, with the v0.5.x cosine-centroid behavior preserved as the fallback path. Same model also classifies source TYPE (academic / article / newsletter / social / journal / reference) at the start of every ingest so the subagent's extraction prompt is tailored to the document's shape. No Anthropic API dependency added — local ONNX only. Prior v0.5.0 ship added the Obsidian brand experience (pre-installed plugin roster, color-coded graph, hotkey scheme, DataviewJS security lockdown, workspace injection to bypass OneDrive conflicts, Web Clipper integration). v0.4.0 added the in-session `paperwik-research` skill. See the `.NOTES` block in `install.ps1` for the full changelog.
