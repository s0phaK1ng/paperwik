...
- **Sections:** minimum 3, **HARD MAXIMUM 5 (v0.7.1)**, paperwik default 3.
  Each section gets its own parallel synthesis subagent later, and the
  total word count divides across sections — so fewer sections = longer
  per-section drafts (up to ~1200 words) and more sections = shorter
  (down to ~600 words). Default to 3 for paperwik (per SKILL.md design
  principle #4: 3 subagents fit comfortably inside a single Pro 5-hour
  prompt window). Raise to 4 or 5 ONLY when the user explicitly asks for
  a longer document or the topic genuinely demands more decomposition.

  **The 5-section ceiling is a budget constraint, not advisory.** v0.7.0's
  D1 run on "LiFePO4 vs Li-ion EV battery chemistry tradeoffs" emitted 8
  sections and overshot the Pro 5-hour-window assumption by ~3×. v0.7.1
  hard-caps at 5. Topics that genuinely need more decomposition should
  be split into multiple research runs, not stuffed into one outline
  beyond the cap.
...
### Mandatory + recommended sections (v0.5.0; tightened in v0.7.1)

Every plan must include:

- **Context** (mandatory, the first section). **The title MUST be exactly
  the literal string `Context`** — no subtitle, no colon, no expansion.
  Examples of WRONG titles the v0.7.1 update prohibits: `Context: Why This
  Comparison Matters`, `Context — Background`, `Setting the Stage`, `Why
  This Topic`, `Introduction`. Examples of CORRECT: `Context`. The output
  validator does an exact-string match against the literal `## Context`
  H2 header. The intent of this section (why this question, what
  triggered it, what decision it informs) is communicated through the
  `intent` field of the outline JSON, not the title.
...
planner does NOT include a `## Verification` appendix; that is also
generated by the Editor from the verification report.

The planner SHOULD include `## Gaps & Caveats` (recommended, not mandatory;
the v1.1 validator emits a WARNING but not an ERROR if absent). **If
included, the title MUST be exactly the literal string `Gaps & Caveats`**
— same constraint as Context, same validator behavior. WRONG examples
prohibited by v0.7.1: `Gaps, Caveats, and Open Questions`, `Caveats`,
`Open Questions`, `Limitations`. CORRECT: `Gaps & Caveats`.

**Topical sections** between Context and Gaps & Caveats can use any
descriptive title — the v1.1 validator allows topic-specific names.
Examples: `Chemistry`, `Energy Density`, `Cost & Supply`, `OEM Adoption`.
Aim for 4-8 words per title.
